...
class Publish extends React.Component {
  ...
  getClusterEnv = () => {
    ...
    this.props.getClusterByCode({}, { code: clusterCode }).then((result) => {
      this.setState({
        showIframes: true,
        clusterEnv: result.env,
      });
    });
  };
  handleOk = (e) => {
  ...
  render() {
          ...
          Modal.confirm({
            ...
            content: (
              <div className="publish-modal-content">
                <p>
                  您确定要发布 <span>{file.name}</span> 到{' '}
                  <span>
                    {URL.parse(window.location.href, true).query.clusterCode}
                  </span>{' '}
                  吗？
                </p>
              </div>
          ...
          if (info.file.response.data.success.length > 0) {
            ...
            notification['error']({
              ...
              description: JSON.stringify(
                info.file.response.data.error[0].message
              ),
              duration: null,
            });
          }
    ...
    if (_.toString(window.hideUpload) !== 'true' && this.state.showIframes) {
      publishTabs.push(
...
module.exports = connect(mapStateToProps, {
  ...
  getClusterByCode: actions.cluster.getClusterByCode,
})(Publish);
